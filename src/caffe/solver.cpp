...
void Solver<Dtype>::Solve(const char* resume_file) {
  ...
  LOG(INFO) << "Size of trainLosses: " << trainLosses_.size();

  //save the losses
  string lossesfilename(param_.snapshot_prefix());
  lossesfilename += "_trainingloss.txt";
  std::ofstream outfile(lossesfilename.c_str());
  for(int i = 0; i < trainLosses_.size(); i++)
  {
    outfile << trainLosses_[i] << std::endl;
  }
  outfile.close();
  LOG(INFO) << "Saving training losses to " << lossesfilename;
}
